...
**Mode Duel à N joueurs** — le Duel récompense **le meilleur**, pas un podium. Une seule règle, valable pour tout `N`, qui se réduit exactement au barème Duo v1 pour `N=2` :

Soit `N` joueurs, chacun avec un écart absolu `e_i = |estimation_i - duration_seconds|`. On identifie le **groupe de tête** : les joueurs ayant le plus petit écart (le meilleur). Soit `k` la taille de ce groupe (nombre d'ex-æquo au meilleur écart).

> **Barème** : le groupe de tête se partage un **pool fixe de 2 points** : chaque membre reçoit `floor(2 / k)`. **Tous les autres joueurs reçoivent 0.** Il n'y a pas de « 2e place » qui marque.

Exemples :
- Duo, écarts distincts : tête `k=1` → `floor(2/1)=2` ; l'autre = 0 → **2, 0**.
- Duo, égalité : tête `k=2` → `floor(2/2)=1` → **1, 1**. **Reproduit exactement le barème v1** (`2/0` ou `1/1`) : zéro régression Duo.
- 4 joueurs, écarts distincts : tête `k=1` → 2 ; les 3 autres = 0 → **2, 0, 0, 0**.
- 4 joueurs, 2 ex-æquo en tête : tête `k=2` → `floor(2/2)=1` → **1, 1, 0, 0**.
- 3 joueurs tous à égalité : tête `k=3` → `floor(2/3)=0` → **0, 0, 0** (personne ne se détache, personne ne marque).

Justification : le Duel v1 attribue les points en jeu (2) au plus proche, partagés en cas d'égalité. Cette formule généralise ce principe unique à N joueurs sans introduire de « rang 2 » (qui aurait cassé le Duo hors égalité) ni de cas particulier `N=2` vs `N≥3`.
...
4. **Duel à N joueurs** : barème par rang d'écart, top 2 seulement (2 pts / 1 pt / 0…) ; en cas d'égalité, les ex-æquo se **partagent le pool** des positions occupées (`floor(somme/k)`), pas de duplication. Se réduit exactement au barème Duo v1 pour N=2 (§1.3).
