...
Rooms Redis + WebSocket pour le jeu type Kahoot (un écran principal/manette hôte, un appareil
par joueur). Redis est branché en **best-effort** : si la connexion échoue au démarrage de
l'API, tout le reste de l'API (pass-and-play, admin, etc.) continue de fonctionner normalement,
seules les routes rooms répondent `503 room_service_unavailable`.

### Modèle hôte (§6.1)

L'**hôte** est le **créateur de la room** (celui qui appelle `POST /rooms`), authentifié par un
`hostToken` secret présenté au premier `join` WS — jamais "le premier joueur connecté" (ancien
modèle). Un seul hôte par room, décidé une fois pour toutes à la création, jamais réattribué à
un join ultérieur même si le créateur tarde à se connecter.

L'hôte choisit, à son premier `join`, s'il **joue en plus de présenter** (`isPlaying: true`,
défaut) ou s'il **présente seulement** (`isPlaying: false`, écran de présentation + contrôleur
pur, modèle Kahoot). Dans tous les cas, l'hôte seul pilote la partie (`mj:start`/`mj:next`/
`mj:skip`). Un hôte non-joueur :
- n'a jamais de zone de réponse (le message `answer` lui est refusé côté serveur, `not_playing`) ;
- n'apparaît **jamais** dans `room:state.players` (jamais dans le classement/la liste affichée) ;
- ne compte jamais dans le calcul "tous les joueurs connectés ont répondu" (le timer/la clôture
  de question n'attend jamais un hôte non-joueur).

Un hôte peut se déconnecter sans interrompre la partie (comportement inchangé, cf.
reconnexion ci-dessous) ; pas de transfert de rôle à un autre joueur.
...
```
{ code: string, hostToken: string, qr: string }  // code = 6 caractères A-Z0-9 (sans 0/O/1/I/L) ; qr = data URL PNG
```
`hostToken` est un secret à usage unique, à présenter par le créateur au premier `join` WS pour
être authentifié comme hôte (voir "Modèle hôte" ci-dessus) — jamais renvoyé par
`GET /rooms/:code` (état public, consultable par n'importe qui avant de rejoindre). `qr` encode
un lien `WEB_BASE_URL/rooms/join?code=XXXXXX` (scan direct, pas juste le code brut).
...
Un seul canal générique sert les rôles hôte (présente et/ou joue, pilote toujours la partie) et
joueur — le rôle hôte est déterminé côté serveur par le `hostToken` (voir "Modèle hôte"
ci-dessus), jamais simplement déclaré par le client. `maxPayload` 16 Ko (anti-DoS). Tant qu'aucun
message `join` n'a été reçu sur une connexion, tout autre message est rejeté (`error`
`unknown_player`).
...
|---|---|---|
| `join` | `code, pseudo, playerId?, hostToken?, isPlaying?` | Rejoint la room. Avec `playerId` (reconnexion) : reprend la place existante (score/rôle inchangés) si l'id est connu de la room, sinon retombe sur un join classique (nécessite alors `pseudo`). Sans `playerId` : nouveau joueur, id généré serveur. `hostToken` correct ET aucun hôte déjà assigné -> ce joueur devient l'hôte ; `isPlaying` (défaut `true`) ne s'applique qu'à lui. |
| `answer` | `binaryAnswer? \| chosenUnit? \| estValue?+estUnit?` | Réponse brute à la question en cours. `questionId`/`roundIndex`/`durationSeconds` ne sont **jamais** envoyés par le client : la room connaît déjà la question courante (vérité terrain chargée serveur à la création). Une seule réponse acceptée par joueur et par question (`already_answered` sinon) ; refusée pour un hôte non-joueur (`not_playing`). |
| `mj:start` | — | Hôte uniquement (`not_host` sinon), qu'il joue ou non. Démarre la partie : 1ère question, arme le timer. |
| `mj:next` | — | Hôte uniquement. Pendant `results` : avance à la question suivante (ou termine la partie si le pool est épuisé). |
| `mj:skip` | — | Hôte uniquement. Pendant `question` : clôture immédiatement, équivalent à l'expiration du timer (débloquer un joueur AFK, §6.3). Traité de façon identique à `mj:next` reçu pendant la phase `question`. |

#### Serveur -> client

| Message | Contenu | Quand |
|---|---|---|
| `room:state` | `code, mode, status, timerSeconds, players[{id,pseudo,isHost,connected,score,hasAnswered}], you:{playerId,isHost,isPlaying}` | Après join/leave/déconnexion/reconnexion/réponse reçue. **Personnalisé par destinataire** (`you`) — jamais un broadcast identique à tous. `players` **exclut tout hôte non-joueur** (jamais dans le classement/la liste affichée) ; `hasAnswered` indique qu'une réponse a été reçue SANS révéler laquelle (§6.1 : jamais de fuite avant révélation, y compris à l'hôte). |
...
| `question:results` | `questionIndex, durationSeconds, results[{playerId,pseudo,points,scoreAfter,binaryAnswer?,chosenUnit?,estValue?,estUnit?,noAnswer}], leaderboard[{playerId,pseudo,score}]` | Question clôturée (tous les joueurs jouants ont répondu, timer expiré, ou `mj:skip`/`mj:next`). Un hôte non-joueur n'apparaît jamais dans `results`/`leaderboard`. Première apparition des réponses de chacun — révélation, jamais avant. |
| `game:end` | `leaderboard[{playerId,pseudo,score}]` | Pool de questions épuisé. Exclut tout hôte non-joueur. |
| `error` | `code: room_not_found\|unknown_player\|not_in_question\|already_answered\|not_host\|not_playing\|invalid_message` | Rejet d'un message client. |
...
- **Timer** (§6.2) : 10 s par défaut, configurable à la création (`POST /rooms` `timerSeconds`).
  ...
  Un joueur **déconnecté** ou un **hôte non-joueur** ne bloque jamais la clôture (seuls les
  joueurs `connected` ET `isPlaying` sont attendus) ; clôture aussi dès que tous les joueurs
  jouants connectés ont répondu, ou sur `mj:skip`.
...
- **Streak** : persiste question après question pendant toute la partie (état en mémoire
  ...
  (`domain/streak.ts::applyAnswer`) — pas de logique de streak dupliquée. Un hôte non-joueur n'a
  jamais de streak (jamais de réponse à scorer).
- **Reconnexion** : le client renvoie `code` + son `playerId` (obtenu au premier `join`,
  ...
  d'hôte, puis renvoie un `room:state` + `question:show` (si une question est en cours) en
  resync. Si le `playerId` est inconnu de la room (TTL expiré) ou absent, un nouveau `pseudo`
  est requis pour rejoindre comme nouveau joueur (`hostToken`/`isPlaying` n'ont plus d'effet à
  ce stade : le rôle d'hôte a déjà été tranché à la création de la room).
- **Concurrence** : les mutations d'une même room (join, réponse, clôture, avance) sont
  sérialisées côté serveur (verrou en mémoire par code de room) — un `join` concurrent sur le
  `hostToken` ne peut jamais produire deux hôtes.
